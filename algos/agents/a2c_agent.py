...
class A2CAgent():
    ...
    def save_model(self, path):
        torch.save(self.actor_net.state_dict(), path+'/actor.bin')
        torch.save(self.critic_net.state_dict(), path + '/critic.bin')

    def load_model(self, path):
        self.actor_net.load_state_dict(torch.load(path + '/actor.bin', map_location=torch.device('cpu')))
        self.critic_net.load_state_dict(torch.load(path + '/critic.bin', map_location=torch.device('cpu')))
